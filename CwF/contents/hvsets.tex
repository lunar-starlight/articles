...
\begin{construction}
  The sum \(A+B\) is given by \(\abs A+\abs B\) with equality
  \begin{align*}
...
\begin{construction}
  ...
  \[ \set{f : \abs A × \abs B → ℋ}{\ref{hvm:str}-\ref{hvm:unique}} \]
  with equality
...
\begin{theorem}
  Morphisms \(f : A ↬ 𝒰\) correspond precisely to maps \(φ : A → 𝒰\)
  satisfying \(φ(x)\res{\eq x {x'}} = φ(x')\res{\eq x {x'}}\).
\end{theorem}
\begin{proof}
  ...
  Therefore the collection \(I\) has at most \(ℋ\) many elements.
