...
\end{construction}
TODO: check if the \(b ∈ B\) should be in the index of the disjunciton. 
\begin{proof}
...
\end{theorem}
\begin{proof}
  Take \(a ∈ A\). The type \(\hat f(a)\) is the gluing of the family
  \(\p{X\res{f(a,X)}}_{X ∈ 𝒰}\), that is the set
  ...
  In the other direction taking a map \(g : A → 𝒰\) we define
  \(g^\flat(a, X) ≔ \e a ∧ \eq X {g(a)}\). This is obviously a morphism
  \(g^\flat : A ↬ 𝒰\).

  If \(f = g : A ↬ 𝒰\), then for \(a ∈ A\) we have
  \[ \hat f(a) = ⋁\p{X\res{f(a, X)}} = ⋁\p{X\res{g(a, X)}} = \hat g(a)\text. \]

  Let \(a ∈ A\) and \(g : A → 𝒰\).
  \begin{align*}
    \hat{g^\flat}(a)
    ≔ ⋁\p{X\res{g^\flat(a, X)}}
    = ⋁\p{X\res{\e a ∧ \eq X {g(a)}}
    = ⋁\p{X\res{\eq X {g(a)}}\res{\e a}
    = ⋁\p{g(a)}\res{\e a}
    = g(a)
  \end{align*}

  It remains to show that \(\hat f^\flat = f\). For \(a ∈ A\) and \(X ∈ 𝒰\) we
  have
  \begin{align*}
    \hat f^\flat(a, X) = \e a ∧ \eq X {\hat f(a)}
  ...
  In the other direction assuming \(\hat f = \hat g : A → 𝒰\) we have at
  \(a ∈ A\) and \(X ∈ 𝒰\)
